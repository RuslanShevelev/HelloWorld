...
if (clientOS == 1) {
    ...
    alert((clientDeviceYear >= 2015) ? 'Установите версию приложения для Android по ссылке' : 'Установите облегченную версию приложения для Android по ссылке');
} else if (clientOS == 0) {
    clientDeviceYear = Number(prompt('Сообщите год выпуска модели вашего телефона'));
    alert((clientDeviceYear >= 2015) ? 'Установите версию приложения для iOS по ссылке' : 'Установите облегченную версию приложения для iOS по ссылке');
} else {
    alert('К сожалению, можно выбрать только из "0" и "1"');
};